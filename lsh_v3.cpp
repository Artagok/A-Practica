...
*/
	...
	for (int i = 0; i < S.size(); i++)
	{
		if ( i + 1 < S.size())
		{
			...
			{
				bool estai = false;
				for (int k = 0; k<buckets[ith_band].size() and not estai; k++){
					if (buckets[ith_band][k]==i) estai = true;
				}
				bool estaj = false;
				for (int k = 0; k<buckets[ith_band].size() and not estaj; k++){
					if (buckets[ith_band][k]==j) estaj = true;
				}

				if(not (estai and estaj)){
					similarity = compute_min_hash(tam,h,S[i],S[j]);
					if (similarity > treshold){
						if (not estai) buckets[ith_band].push_back(i);
						if (not estaj) buckets[ith_band].push_back(j);
					}
				}
			}
		}
